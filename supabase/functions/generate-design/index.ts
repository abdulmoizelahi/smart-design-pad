...
serve(async (req) => {
  ...
  try {
    const { plotLength, plotWidth, rooms, style } = await req.json();
    console.log('Generating design for:', { plotLength, plotWidth, rooms, style });
    ...
    const prompt = `Create a professional architectural floor plan for a ${style} style house. 
    ...
    Include bedroom(s), bathroom(s), kitchen, living room, and other necessary spaces.
    Show proper spacing and realistic room proportions.`;
